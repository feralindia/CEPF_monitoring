...
for (i in 1: nrow(grp)){ # create loop to extract median NDVI, changed from average to median to avoid outliers
    ## without gap fill
    ...
    if(length(sb.grp)>1) {
        ...
        execGRASS("r.series",
                  flags=c("z", "overwrite"),
                  parameters=list(input=tmp.input, output=avg.ndvi, method='median')) ## changed method from average to median Apr '15
    } else {
    ...
    if(length(gf.regmap)>0){
        ...
        if(length(gf.regmap>1)){
            ...
            execGRASS("r.series",
                      flags=c("z", "overwrite"),
                      parameters=list(input=gf.tmp.input, output=gf.avg.ndvi, method='median'))
